...
from cfg import DATA_FOLDER, DEFAULT_PORT, STAT_DATA_FOLDER, WEEK_NUMBER_FILE, DEFAULT_LANG, LANGS
...
class FileManager():
    ...
    @classmethod
    def __setGuildData(cls, guild_id: int, manager_role_id: int, channel_id: int, message_id: int, server_ip: str, port: str, lang: str):
    ...
    def setRole(cls, guild_id: int, new_manager_role: Role):
        ...
        if not data:
            return False
        cls.__setGuildData(guild_id, new_manager_role.id, data["channel"], data["message"], data["server_ip"], data["port"], data["lang"])

    @classmethod
    def setChannel(cls, guild_id: int, new_channel: TextChannel):
        """Sets channel for guild. Returns False if file is not present"""
        data = cls.getGuildData(guild_id)
        if not data:
            return False
        cls.__setGuildData(guild_id, data["mgr_role"], new_channel.id, data["message"], data["server_ip"], data["port"], data["lang"])

    @classmethod
    def setMessage(cls, guild_id: int, new_message: Message):
        """Sets channel for guild. Returns False if file is not present"""
        data = cls.getGuildData(guild_id)
        if not data:
            return False
        cls.__setGuildData(guild_id, data["mgr_role"], data["channel"], new_message.id, data["server_ip"], data["port"], data["lang"])
    ...
    def setServerIp(cls, guild_id: int, new_ip: str):
        ...
        if not data:
            return False
        cls.__setGuildData(guild_id, data["mgr_role"], data["channel"], data["message"], new_ip, data["port"], data["lang"])
    ...
    def setPort(cls, guild_id: int, new_port: str):
        ...
        if not data:
            return False
        cls.__setGuildData(guild_id, data["mgr_role"], data["channel"], data["message"], data["server_ip"], new_port, data["lang"])
    ...
    def setLangRu(cls, guild_id: int):
        ...
        if not data:
            return False
        cls.__setGuildData(guild_id, data["mgr_role"], data["channel"], data["message"], data["server_ip"], data["port"], LANGS["russian"])
    ...
    def setLangEn(cls, guild_id: int):
        ...
        if not data:
            return False
        cls.__setGuildData(guild_id, data["mgr_role"], data["channel"], data["message"], data["server_ip"], data["port"], LANGS["english"])

    @classmethod
    def resetChannel(cls, guild_id: int):
        """Sets channel for guild. Returns False if file is not present"""
        data = cls.getGuildData(guild_id)
        if not data:
            return False
        cls.__setGuildData(guild_id, data["mgr_role"], -1, data["message"], data["server_ip"], data["port"], data["lang"])

    @classmethod
    def resetMessage(cls, guild_id: int):
        """Sets channel for guild. Returns False if file is not present"""
        data = cls.getGuildData(guild_id)
        if not data:
            return False
